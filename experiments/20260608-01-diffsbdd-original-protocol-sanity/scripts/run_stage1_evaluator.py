#!/usr/bin/env python3
"""Run evaluator wiring for the DiffSBDD stage 1 sanity trial."""
...
BASELINE_NAME = "diffsbdd_stage1_original_protocol_sanity"
SUMMARY_NOTE = (
    "Stage 1 evaluator wiring summary only. No formal labels, prevalence, "
    "cross-method comparison, or repair benchmark claims."
)
POSEBUSTERS_INNER_TIMEOUT_SECONDS = 30
POSEBUSTERS_OUTER_TIMEOUT_SECONDS = 45
...
def run_posebusters(sample: dict[str, Any], evaluator_input: Path, index: int, config: str, raw_dir: Path) -> dict[str, Any]:
    ...
    command = [
        ...
        str(output_path),
        "--timeout",
        str(POSEBUSTERS_INNER_TIMEOUT_SECONDS),
        "--config",
        config,
    ]
    metrics: dict[str, Any] = {
    ...
    if output_path.exists():
        ...
        try:
            result = subprocess.run(
                ...
                text=True,
                cwd=ROOT,
                timeout=POSEBUSTERS_OUTER_TIMEOUT_SECONDS,
            )
            ...
            metrics["timeout_seconds"] = POSEBUSTERS_OUTER_TIMEOUT_SECONDS
            errors.append(f"timeout_after_{POSEBUSTERS_OUTER_TIMEOUT_SECONDS}s")
            return tool_row(
                sample,
    ...
    if (exit_code not in (0, None)) or metrics.get("posebusters_error"):
        status = "tool_failure"
        errors.append(str(metrics.get("posebusters_error") or f"exit_code_{exit_code}"))
...
def build_diagnosis_rows(samples: list[dict[str, Any]], tool_rows: list[dict[str, Any]], tool_results_path: Path) -> list[dict[str, Any]]:
    ...
    for sample in samples:
        ...
        elif any(row["status"] in {"tool_failure", "timeout"} for row in core):
        ...
        diagnosis_rows.append(
            {
                ...
                "evidence": {
                    ...
                    "claim_boundary": "stage 1 sanity only, no formal labels generated",
                },
                ...
                "notes": "Stage 1 evaluator wiring only. Vina, if present, is auxiliary score-only evidence.",
            }
        )
...
def update_stage_attrition(path: Path, diagnosis_rows: list[dict[str, Any]], tool_rows: list[dict[str, Any]]) -> None:
    ...
    if rows:
        ...
        row["N_docking_failed"] = sum(1 for item in dock_rows if item["status"] in {"failed", "tool_failure", "timeout"})
    ...
    payload["mvp_required_counts"]["N_tool_failure"] = sum(1 for item in tool_rows if item["status"] in {"tool_failure", "timeout"})
...
def main() -> int:
    ...
    if generation_summary.exists():
        ...
        payload.setdefault("notes", "Stage 1 generation/output-capture summary only. No formal labels or prevalence claims.")
