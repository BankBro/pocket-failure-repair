...
- 术语表达: 能用中文准确表达的内容尽量使用中文; 仅在路径、命令、参数名、字段名、环境名、schema 名、工具名、模型名或通用技术缩写需要保留原文时使用英文.
...
- 关键状态文档: `docs/STATUS.md` 只记录当前最重要的项目快照, 用于上下文压缩或新会话后的快速恢复; 新会话或上下文压缩后恢复工作时, 先阅读 `docs/STATUS.md`, 再按其中的当前阶段、下一步和暂停条件继续; `docs/EXPERIMENT_LOG.md` 追加记录完整实验、调研和阶段性工作过程, 包括输出、结论、失败原因和下一步.
- 中间产物管理: 需要创建临时脚本或中间文件时, 放在 `tmp/YYYYMMDD-<task-slug>/`, 其中 `<task-slug>` 使用简短英文 kebab-case; 任务结束后删除该子目录及其内容, 不把 `tmp/` 当长期资产目录.
- 大文件下载规范: 下载数据集、checkpoint 或其他大文件时, 优先使用支持稳定断点续传的 `aria2c`, 例如 `aria2c -c -x 8 -s 8 -k 1M --retry-wait=30 --max-tries=0 -o <filename> "<url>"`; 若 `aria2c` 不可用, 再使用 `wget -c`. 下载完成后记录来源 URL、访问时间、文件大小、checksum、下载日志和保留/删除策略; 压缩包如需删除以节省空间, 必须先完成 checksum 校验和解压记录.
...
- Metadata / config schema 规范: 新增项目自有 JSON / JSONL / YAML metadata 或配置时, 先判断归属并查 `schemas/README.md`: config schema 放 `schemas/configs/`, canonical data schema 放 `schemas/data/`, 第三方 audit 输出 schema 放 `schemas/third_party_audit/`. 若已有 schema, 必须按 schema 写入字段, 并记录 `schema_version` 和 `schema_path`; 新增或迁移写出代码时优先使用 `pfr.utils.schema_io` 或 `scripts.eval.audit_common` 暴露的 schema-aware writer 自动注入 schema refs, 不在多个脚本复制双常量. 明确例外包括 Conda environment 文件和外部工具原生输出. schema 是版本化合同, 已被实验使用的版本不要静默改语义; 改 required 字段、字段类型、字段含义或统计口径时应新增 schema 版本.
